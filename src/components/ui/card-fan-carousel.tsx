  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[18rem] sm:h-[22rem] md:h-[26rem] lg:h-[30rem] xl:h-[34rem] overflow-visible">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden">
                <img src={card.imgUrl} referrerPolicy="no-referrer" loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer absolute w-[9rem] h-[14rem] sm:w-[11rem] sm:h-[17rem] md:w-[13rem] md:h-[20rem] lg:w-[15rem] lg:h-[24rem] xl:w-[17rem] xl:h-[27rem] rounded-2xl overflow-hidden border border-[#1a1a1a]/15 shadow-xl bg-white shrink-0 origin-bottom" onClick={card.onClick}>{image}</a>
            ) : (
              <div key={index} onClick={card.onClick} className="fan-card absolute w-[9rem] h-[14rem] sm:w-[11rem] sm:h-[17rem] md:w-[13rem] md:h-[20rem] lg:w-[15rem] lg:h-[24rem] xl:w-[17rem] xl:h-[27rem] rounded-2xl overflow-hidden border border-[#1a1a1a]/15 shadow-xl bg-white shrink-0 origin-bottom cursor-pointer">{image}</div>
            );
          })}
        </div>
      </div>
