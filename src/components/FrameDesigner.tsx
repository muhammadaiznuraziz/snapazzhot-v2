...
export default function FrameDesigner({
  ...
  // Track temporary preview Blob URLs to clean them up safely
  ...
  useEffect(() => {
    return () => {
      if (previewUrlsRef.current.framePng?.startsWith("blob:")) {
        URL.revokeObjectURL(previewUrlsRef.current.framePng);
      }
      if (previewUrlsRef.current.backgroundImage?.startsWith("blob:")) {
        URL.revokeObjectURL(previewUrlsRef.current.backgroundImage);
      }
    };
  }, []);
  ...
  useEffect(() => {
    if (initialTemplate) {
      ...
      // Default initial templates layout
  ...
  const addElement = (type: FrameElement["type"], customName?: string) => {
    ...
    switch (type) {
      case "photo":
        const photoNum =
          elements.filter((el) => el.type === "photo").length + 1;
  ...
  // Upload frame template images - Fixed blob leaking & async publicUrl replacement
  const handleUploadImage = async (
    ...
    // Create temporary Blob URL for immediate visual feedback
    const temporaryLocalUrl = URL.createObjectURL(file);

    // Track old blob URL to revoke later
    const key = isFramePng ? "framePng" : "backgroundImage";
    const oldBlobUrl = previewUrlsRef.current[key];
    previewUrlsRef.current[key] = temporaryLocalUrl;

    if (isFramePng) {
      setFramePng(temporaryLocalUrl);

      // Auto-adjust canvas size to match PNG dimensions
      const img = new Image();
      img.onload = () => {
        if (img.naturalWidth > 0 && img.naturalHeight > 0) {
          ...
          showToast(
            `Ukuran kanvas: ${img.naturalWidth} x ${img.naturalHeight}px`,
          );
        }
      };
      img.src = temporaryLocalUrl;
    } else {
      setBackgroundImage(temporaryLocalUrl);
    }

    // Revoke previous temporary blob URL if exists
    if (oldBlobUrl && oldBlobUrl.startsWith("blob:")) {
      URL.revokeObjectURL(oldBlobUrl);
    }

    showToast("Gambar tampil di canvas! Menyimpan ke Supabase...");

    try {
      const fileExt = file.name.split(".").pop() || "png";
      const fileName = `${Date.now()}-${isFramePng ? "frame-overlay" : "background"}.${fileExt}`;
      const filePath = `templates/${fileName}`;

      // Directly upload File object to Supabase Storage (Faster & reliable)
      ...
      if (uploadError) {
        // Detect RLS / permission errors and provide actionable guidance
        const msg = (uploadError.message || "").toLowerCase();
        if (
          msg.includes("row-level security") ||
          msg.includes("row violates") ||
          msg.includes("policy") ||
          msg.includes("permission") ||
          msg.includes("unauthorized")
        ) {
          console.warn("RLS policy error:", uploadError);
          showToast(
            "⚠️ Upload ditolak oleh Storage RLS Policy. Buka Supabase Dashboard > SQL Editor, buka file supabase/storage-rls-policies.sql, lalu jalankan seluruh script. (Pastikan bucket 'photobooth-media' sudah public).",
          );
          throw new Error(
            "Storage RLS policies missing. Run supabase/storage-rls-policies.sql in Supabase SQL Editor.",
          );
        }
        throw uploadError;
      }
      ...
      if (publicUrl) {
        if (isFramePng) {
          setFramePng(publicUrl);
        } else {
          setBackgroundImage(publicUrl);
        }

        // Cleanup temporary blob URL now that publicUrl is active
        if (temporaryLocalUrl.startsWith("blob:")) {
          URL.revokeObjectURL(temporaryLocalUrl);
          delete previewUrlsRef.current[key];
        }

        showToast("Gambar berhasil diunggah ke Supabase Storage!");
      } else {
        showToast("Gagal mendapatkan URL publik, gambar lokal tetap aktif.");
      }
    } catch (err: any) {
      console.error("Upload failed:", err);
      showToast(
        `Gagal upload ke Supabase: ${err?.message || "Unknown error"}. Gambar lokal tetap aktif.`,
      );
    }
  };
  ...
  const handleSave = async () => {
    ...
    const actualPhotoCount = elements.filter(
      (el) => el.type === "photo",
    ).length;
    if (actualPhotoCount === 0) {
      showToast("Template harus memiliki minimal 1 slot foto.");
      return;
    }

    // Safeguard: Ensure no transient blob URLs get persisted to the DB
    if (framePng.startsWith("blob:")) {
      showToast("Tunggu hingga proses upload frame selesai sebelum menyimpan.");
      return;
    }
    if (backgroundImage.startsWith("blob:")) {
      showToast(
        "Tunggu hingga proses upload background selesai sebelum menyimpan.",
      );
      return;
    }
    ...
    const templateId =
      initialTemplate?.id ||
      `tpl-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    ...
    try {
      ...
      if (res) {
        ...
        if (
          testErr &&
          (testErr.message?.includes("permission") ||
            testErr?.message?.includes("policy"))
        ) {
          showToast("Gagal: Izin database (RLS) belum diatur. Hubungi admin.");
        } else if (testErr) {
          showToast(`Gagal menyimpan: ${testErr.message || "Database error"}`);
        } else {
          showToast("Gagal menyimpan template frame. Silakan coba lagi.");
        }
      }
    } catch (err: any) {
      console.error("Save template error:", err);
      showToast(
        `Error: ${err?.message || "Unknown error saat menyimpan template"}`,
      );
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-400">
              Atur tata letak & frame satu kali untuk semua pengunjung
            </p>
          </div>
        </div>
        ...
        <aside className="w-80 border-r border-zinc-800 bg-zinc-950 flex flex-col overflow-y-auto shrink-0 select-none">
          ...
          <div className="p-4 border-b border-zinc-900 space-y-3.5">
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-zinc-500 font-semibold block">
                Nama Template
              </label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              ...
              <div className="space-y-1">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-2 text-xs text-center font-bold text-blue-400">
                  {elements.filter((el) => el.type === "photo").length} Slot
                  Foto
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {framePng && (
                <div className="flex items-center gap-2 mt-1 bg-zinc-900/45 border border-zinc-850/40 p-1.5 rounded-lg">
                  ...
                  />
                  <span className="text-[9px] font-mono font-bold text-zinc-300 w-7 text-right">
                    {frameOpacity}%
                  </span>
                </div>
              )}
          ...
          <div className="h-12 border-b border-zinc-800 bg-zinc-950 px-6 flex items-center justify-between select-none shrink-0">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <span
                  className={`font-mono text-[10px] px-1.5 py-0.5 rounded ${snapToGrid ? "bg-blue-500/20 text-blue-300" : "bg-zinc-950 text-zinc-500"}`}
                >
                  {gridSize}%
                </span>
              </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono font-bold w-12 text-center text-zinc-400 bg-zinc-900 border border-zinc-850 px-2 py-1 rounded-md">
                {Math.round(zoom * 100)}%
              </span>
              <button
          ...
          >
            ...
            >
              ...
              >
                      ...
                      >
                        {elem.type === "photo" && (
                          <div className="text-center p-2 font-bold text-white uppercase flex flex-col items-center justify-center w-full h-full">
                            ...
                            <span className="text-[10px] font-extrabold tracking-wider text-zinc-200 select-none drop-shadow">
                              {elem.name}
                            </span>
                          </div>
                        )}
                        ...
                        {elem.type === "logo" && (
                          <div className="text-center p-1 text-[9px] text-emerald-400 font-extrabold tracking-wider uppercase flex flex-col items-center justify-center">
                            ...
                            <span className="drop-shadow text-zinc-300 truncate max-w-full">
                              {elem.name}
                            </span>
                          </div>
                        )}
                        ...
                        {elem.type === "text" && (
                          <div className="px-3 py-1 w-full h-full flex items-center justify-center text-center">
                            <span
                              style={{
                                fontSize: `${Math.max(8, (elem.fontSize || 14) / 1.5)}px`,
                        ...
                        {isSelected && !elem.locked && (
                          <>
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "n", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "s", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "e", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "w", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "nw", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "ne", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "se", elem)
                              }
                            ...
                            <div
                              onMouseDown={(e) =>
                                handleResizeMouseDown(e, "sw", elem)
                              }
                ...
                {/* Frame transparent overlay image */}
                {framePng && (
                  <img
                    src={framePng}
                    alt="Frame overlay"
            ...
            {selectedElem ? (
              <div className="space-y-4">
                <div className="flex justify-between items-center bg-zinc-900 p-2.5 rounded-lg border border-zinc-850">
                  <span className="text-xs font-extrabold text-blue-400 truncate">
                    {selectedElem.name}
                  </span>
                ...
                <div className="space-y-3 pt-1">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] text-zinc-400">
                      <span>Tinggi:</span>
                      <span className="font-mono text-white">
                        {Math.round(selectedElem.height)}%
                      </span>
                    </div>
                    <input
                      type="range"
                ...
                {selectedElem.type !== "photo" && (
                  <div className="space-y-2 pt-3 border-t border-zinc-900">
                    <div className="flex items-center justify-between">
                      <label
                        className="text-[11px] text-zinc-400 font-semibold cursor-pointer select-none flex items-center gap-2"
                        htmlFor="renderOnTop-checkbox"
                      >
                    ...
                    <p className="text-[9px] text-zinc-500 leading-normal">
                      Jika diaktifkan, elemen ini akan diposisikan di atas
                      gambar bingkai PNG transparan (overlay) sehingga tidak
                      tertutup.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2 pt-3 border-t border-zinc-900">
                  <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">
                    Z-Index (Urutan Layer)
                  </h4>
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        updateElement(selectedElem.id, {
                          zIndex: Math.max(1, selectedElem.zIndex - 1),
                        })
                      }
              ...
              <div className="py-12 text-center text-zinc-500 text-xs">
                Silakan pilih salah satu elemen di canvas untuk melakukan
                konfigurasi detail.
              </div>
            )}
          </div>
