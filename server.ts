...
const STORAGE_BUCKET = process.env.SUPABASE_STORAGE_BUCKET || "photobooth-media";


const app = express();
const PORT = 3000;

// Increase limit to allow base64 uploads (important for photo booth captures)
app.use(express.json({ limit: "50mb" }));
app.use(express.urlencoded({ limit: "50mb", extended: true }));

// Default in-memory values are used only until the first Supabase read completes.
// They are never written to local files.
...
const defaultDb: DatabaseSchema = {
  ...
  events: [
    {
      ...
      countdown: 5,
      photoCount: 4,
      layoutType: "strip", // 'strip' (2x6), 'grid' (4x6), 'single' (4x6 single)
      themeColor: "#db2777",
      qrExpiredMinutes: 60,
      emailTemplate:
        "Thank you for attending our special day! Download your photo using the link below.",
    },
    {
      id: "evt-graduation",
      ...
      emailTemplate:
        "Congratulations, Graduate! Here is your professional photo booth memory.",
    },
    {
      id: "evt-corporate",
      name: "Summit 2026 Gala Dinner",
      logo: "🚀 Enterprise Summit",
      ...
      qrExpiredMinutes: 30,
      emailTemplate:
        "Thank you for joining the Summit 2026 Gala. View your keepsake here.",
    },
  ],
  photos: [],
  settings: {
  ...
  printer: {
    ...
    status: "Online", // Online, Offline, Busy, Paper Empty, Ink Low
    paperRemaining: 142,
    inkLevel: 85,
    totalPrints: 1240,
...
let memoryDb: DatabaseSchema = structuredClone(defaultDb);

// Helper functions to map camelCase <-> snake_case for PostgreSQL compatibility
...
function mapKeys(obj: any, fn: (key: string) => string): any {
  ...
  if (obj !== null && typeof obj === 'object') {
    return Object.keys(obj).reduce((acc, key) => {
      const value = obj[key];
      // Keep JSON fields like elements, layoutPositions, or meta intact without key mapping
...
async function syncSettingsToSupabase(settings: any) {
  if (!supabase) return;
  try {
...
// Supabase is the persistent data source. The memory object only keeps a
// request-local cache so existing simulator endpoints can operate synchronously.
function readDb() {
  return memoryDb;
}
...
type UploadPayload = { buffer: Buffer; contentType: string; extension: string };
...
function decodeUploadPayload(value: string, fallbackContentType: string): UploadPayload {
...
// Template frames and backgrounds are storage assets, not gallery photos.
...
// Helper: Add log
function addActivityLog(type: string, message: string) {
  const db = readDb();
  db.logs.unshift({
    id: `log-${Date.now()}`,
...
// API: AUTHENTICATION (Simulated Simple JWT/Admin authentication)
app.post("/api/auth/login", (req, res) => {
  const { username, password } = req.body;
  // Simple operator or admin credentials
  if (
...
// API: UPDATE PHOTO SETTINGS FOR GALLERY
app.put("/api/gallery/:id", (req, res) => {
  ...
  if (photo) {
    ...
    if (username !== undefined) photo.username = username || "Guest";
    if (templateName !== undefined) photo.templateName = templateName;
    if (likeCount !== undefined) photo.likeCount = likeCount;
    writeDb(db);
    res.json({ success: true, data: photo });
  } else {
    res.status(404).json({ success: false, message: "Photo not found" });
...
app.post("/api/upload", async (req, res) => {
  try {
    const { file, type, eventId, meta } = req.body; // file is base64 string, type: 'photo' | 'gif' | 'video'
    if (!file) {
      return res
        .status(400)
        .json({ success: false, message: "No file content provided" });
    }
    ...
    // Process and save individual raw photos if present in meta
    const savedRawPhotos: string[] = [];
    const rawPhotoPaths: string[] = [];
    ...
    // Save loop GIF from base64 if present in meta
    if (
      meta &&
      meta.gifUrl &&
      typeof meta.gifUrl === "string" &&
      meta.gifUrl.startsWith("data:")
    ) {
      try {
    ...
    // Save BTS Video from base64 if present in meta
    if (
      meta &&
      meta.videoFile &&
      typeof meta.videoFile === "string" &&
      meta.videoFile.startsWith("data:")
    ) {
      try {
    ...
    // Remove raw videoFile base64 from metadata to avoid database bloat
    delete cleanMeta.videoFile;

    // Save No-Frame Collage from base64 if present in meta
    if (
      meta &&
      meta.noFrameFile &&
      typeof meta.noFrameFile === "string" &&
      meta.noFrameFile.startsWith("data:")
    ) {
      try {
    ...
    // Remove raw noFrameFile base64 from metadata to avoid database bloat
    ...
    const newPhoto = {
      ...
      likeCount: 0,
      mirror_enabled:
        cleanMeta.mirror_enabled !== undefined
          ? cleanMeta.mirror_enabled
          : false,
      meta: cleanMeta,
    };
...
app.post("/api/print", (req, res) => {
  ...
  if (db.printer.status === "Offline") {
    return res
      .status(400)
      .json({ success: false, message: "Printer offline!" });
  }
  ...
  // Update photo prints count
  const photoIndex = db.photos.findIndex((p) => p.id === photoId);
  if (photoIndex !== -1) {
    db.photos[photoIndex].printsCount =
      (db.photos[photoIndex].printsCount || 0) + numCopies;
  }
...
// API: CAMERA SIMULATOR (DSLR capture simulation & status)
...
app.post("/api/camera/connect", (req, res) => {
  ...
  const db = readDb();
  const camName =
    cameraId === "cam-dslr-2" ? "Sony Alpha a6400" : "Canon EOS 80D (DSLR)";
...
// DSLR Simulated capture with multiple stylized preloaded dummy models
app.post("/api/camera/capture", (req, res) => {
  const db = readDb();
  if (db.camera.status === "Disconnected") {
    return res
      .status(400)
      .json({ success: false, message: "Kamera terputus!" });
  }

  // Pre-configured high quality portrait image list (unreferring to direct assets since we can use robust, royalty free AI styled portraits or beautiful canvas generators)
  // We'll generate dynamic retro colors or patterns to make it unique and high fidelity in the interface.
...
app.get("/api/analytics", (req, res) => {
  ...
  // Create a 7-day activity chart dummy data based on actual photos count
  const activityData = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    const dateStr = d.toLocaleDateString("id-ID", { weekday: "short" });
    // Match actual photos on this day
    ...
    activityData.push({
      name: dateStr,
      Foto: dayPhotos + (i === 0 ? 0 : Math.round(Math.random() * 8 + 2)), // add some random for history to make chart beautiful, but keep today's real
